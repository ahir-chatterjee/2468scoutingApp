...
class WebInputViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let text = """
        (#\(GlobalState.match),\(GlobalState.team)) \(GlobalState.penalty == "Yes" ? 1 : 0) \(GlobalState.line == "Yes" ? 1 : 0) \(GlobalState.autoswitch) \(GlobalState.autoscale) \(GlobalState.pickerView) \(GlobalState.allianceswitch) \(GlobalState.opposingswitch) \(hangTime) \(GlobalState.assistView) \(GlobalState.vault) | \(arrayString) | \(GlobalState.comment)
        """
